...
public class ChooseActionBtnView : SelfAdaptionChooseBtnView
{
    ...
    public UInt64 appliedActionWithPeopleOnlyId;//和谁一起
    People withPeople = null;
    public override void Init(params object[] args)
    {
        base.Init(args);
        int theId = (int)args[1];
        appliedActionWithPeopleOnlyId = (UInt64)args[2];
        ...
        {
            withPeople = RoleManager.Instance.FindPeopleWithOnlyId(appliedActionWithPeopleOnlyId);
            txt.SetText("和" + withPeople.protoData.Name + "一起" + actionSetting.name);
        }
